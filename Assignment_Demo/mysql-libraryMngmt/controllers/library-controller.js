    ...
    try {
        ...
        bookdb.query(sql, (err, results)=>{
            if(err) console.log('query' + err);
                ...
                if(results.affectedRows > 0) res.json({"message" : `${category} is added to the database with ID ${results.insertId}`});
                else res.json({"message":"Failed to add, invalid or missing credentials"});
            }
        });
    ...
    try {
        ...
        bookdb.query(sql, (err, results)=>{
            ...
            if(err) console.log('query' + err);
                ...
                if(results.affectedRows !== 0) res.json({"message" : `${category} with ID ${id} is deleted from the database`});
                else res.json({"message":"Failed to delete, invalid ID"});
            }
        });
    } catch (error) {
        console.error(error);
